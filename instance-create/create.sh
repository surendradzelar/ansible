...
Instance_Create() {
  ...
  aws route53 change-resource-record-sets --hosted-zone-id Z05773462ZI97WYIPLSZ9 --change-batch file:///tmp/record.json
  sed -i -e "/${COMPONENT}/ d" ../inv
  echo "${IPADDRESS} COMPONENT=$(echo ${COMPONENT} | awk -F - '{print $1}')" >>../inv
}

if [ "$1" == "all" ]; then
  for instance in frontend1 login1 todo1 redis1 users1 ; do
